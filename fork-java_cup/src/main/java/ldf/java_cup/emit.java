...
//                        ").value != null )");
  ...
  protected static void do_table_as_string(PrintWriter out, short[][] sa) {
    ...
    nchar  =do_newline(out, nchar, nbytes);
    for (short[] sa_ : sa) {
      nbytes += do_escaped(out, (char) (sa_.length >> 16));
      nchar = do_newline(out, nchar, nbytes);
      nbytes += do_escaped(out, (char) (sa_.length & 0xFFFF));
      nchar = do_newline(out, nchar, nbytes);
      for (short sa__ : sa_) {
        // contents of string are (value+2) to allow for common -1, 0 cases
        // (UTF-8 encoding is most efficient for 0<c<0x80)
        nbytes += do_escaped(out, (char) (2 + sa__));
        nchar = do_newline(out, nchar, nbytes);
      }
    }
    out.print("\" }");
  ...
  public static void parser(
    ...
    throws internal_error
    {
      ...
      out.println("public class " + parser_class_name + typeArgument() +
                  " extends ldf.java_cup.runtime.lr_parser {");
      ...
      if (!suppress_scanner) {
          ...
          out.println("  public " + parser_class_name +
                      "(ldf.java_cup.runtime.Scanner s) {super(s);}");
          // TUM 20060327 added SymbolFactory aware constructor
          out.println();
          out.println("  /** Constructor which sets the default scanner. */");
          out.println("  public " + parser_class_name +
                      "(ldf.java_cup.runtime.Scanner s, ldf.java_cup.runtime.SymbolFactory sf) {super(s,sf);}");
      }
      ...
      out.println("  public ldf.java_cup.runtime.Symbol do_action(");
      out.println("    int                        act_num,");
      out.println("    ldf.java_cup.runtime.lr_parser parser,");
      ...
      if (scan_code != null)
        {
          ...
          out.println("  public ldf.java_cup.runtime.Symbol scan()");
          out.println("    throws java.lang.Exception");
          out.println("    {");
